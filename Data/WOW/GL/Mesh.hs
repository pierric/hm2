...
newMesh :: M2World m f GLResource => M2Model -> m Mesh
newMesh mdl = liftIO $ do
                -- main part of this routine is to generate and fill an OpenGL vertex buffer
                ...
                GL.withMappedBuffer GL.ArrayBuffer GL.WriteOnly
                         (\ptr -> let pos = map (to4 . vd_pos_) (m_vertices_ mdl)
                                      nor = map (vd_normal_)    (m_vertices_ mdl)
    ...
    where
      ...
      bw vert = case zip (map fromIntegral $ vd_bones_ vert) (map fromIntegral $ vd_weights_ vert) of
                  [x,y,z,w] -> (x,y,z,w)
                  _         -> error "a vertex not affected by four bones"

-- Apply the skeleton to the mesh, where the skeleton is simply a matrix for each bone.
skeletonAnim :: M2World m f GLResource => [Matrix] -> Mesh -> m Mesh
skeletonAnim bones mesh = 
    liftIO $ do 
      -- number of vertices and the vertex buffer
...
withMaterial :: M2World m f GLResource  => [Maybe ResourceId] -> Mesh -> RenderPass -> IO () -> m ()
withMaterial rep_tex mesh rp act = 
    do let tex = case textures_ mesh !! r_tex_ rp of         
       ...
       tf <- liftM (join . listToMaybe) $ mapM findResource $ maybeToList tex
       case tf of
        Just (GLTexture (GLTypes.Texture txtyp txobj))
             -> liftIO $ do case r_blendmode_ rp of
                              0 -> return ()
        ...
        _
             -> liftIO $ do GL.blend GL.$= GL.Disabled
                            GL.alphaFunc GL.$= Nothing
                            GL.textureBinding GL.Texture2D GL.$= Nothing
                            act
 
-- renderAll is for test purpose. It simplely renders every submesh
renderAll :: M2World m f GLResource => [Maybe ResourceId] -> Mesh -> m ()
