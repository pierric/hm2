{-# LANGUAGE TypeFamilies,TemplateHaskell, BangPatterns #-}
...
fieldS f r@(Record d o) = let s = fromIntegral $ fieldI f r
...
newDBC rid = do
  ...
  assert (hd == 1128416343) $ return $ DbcDesc (fromIntegral nr) (fromIntegral sr) dat str
...
instance DBC CreatureSkinDB where
    data Field CreatureSkinDB = CreatureSkinID | CreatureSkinModelID | CreatureSkinNPCID
                              | CreatureSkin0 | CreatureSkin1 | CreatureSkin2
